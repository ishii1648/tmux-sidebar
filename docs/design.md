...
| `internal/tmux` | tmux コマンド実行、session/window/pane 情報取得、mutate 操作（kill/rename/swap/move/new）|
| `internal/state` | `/tmp/agent-pane-state` 形式の読み取り、unread/履歴の管理 |
| `internal/config` | hidden/pinned/muted/order/width の読み込みと書き込み |
...
inline rename / move-mark / multi-select / confirm dialog は normal mode の **sub-state** として表現する（モード追加ではなく内部フラグ）。これにより key dispatch ロジックを単純に保つ。
...
| `R`（window rename） | `rename-window -t <session>:<index> <name>` |
| `Shift+R`（session rename） | `rename-session -t <session> <name>` |
...
| `Alt+J`/`Alt+K`（session 並べ替え） | `session_order` ファイル更新（tmux primitive なし） |
...
`R` 押下時、対象行を `bubbles/textinput` に差し替えて編集可能にする。
- `Enter` で `rename-window` 発行 + 通常表示に戻る
- `Esc` で取消（変更なし）
- 入力中は cursor 移動・他コマンドを無効化（rename mode フラグで分岐）
...
### session 並べ替え (`Alt+J/K`)

tmux に session 並び順は無いため、`~/.config/tmux-sidebar/session_order` に
session 名の行リストを保存し、loadData で `sessionOrder` をこのファイル順で再構成する。
ファイル未記載の session はファイル末尾に creation 順で追加する。
...
type Config struct {
    ...
    SessionOrder   []string  // unpinned 群の表示順
    Width          int
}
```
...
3. `pinned` は `PinnedSessions` の順で並べ、unpinned は `SessionOrder` で並べる（未記載は末尾）
4. pinned/unpinned の間に区切り線を挿入
5. `muted` フラグは ListItem に伝播し、render 時に badge を抑制

pin/mute toggle 時は対応ファイルを書き戻し、loadData を再発火する。
...
`--context=<file>` は temp file path で、現在の session 一覧 / pinned / muted / sidebar の
