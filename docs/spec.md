...
running バッジは **1 ターンの間ずっと点灯** し続ける。Claude は tool 実行と応答生成を行き来するが、その全体が作業中なのでバッジは running のまま維持され、tool 完了ごとに idle へ戻る flicker は起こさない。idle になるのはターン終了（ユーザ応答待ち）のときだけ。経過時間も同様に **1 ターンの開始からの累積** で表示し、permission / ask の待機を挟んでもリセットされない。起点は **新しいユーザ入力ごとに張り直される** ため、前のターンを途中で中断（Esc）したり Claude がクラッシュしたりして終えても、次のターンの経過時間に前ターンの離席時間が混ざって巨大な値になることはない。
...
## Agent View (`claude agents`) との分担

Claude Code v2.1.139+ で導入された **Agent View** (`claude agents`) は、tmux に紐付かない supervisor 経由のバックグラウンドセッションを 1 画面で監視・ディスパッチする UI である。tmux-sidebar とはスコープと責務を分けて並走させる。

| ツール | 役割 | dispatch スコープ | 対応 launcher |
|---|---|---|---|
| `tmux-sidebar new` (popup picker) | **repo 横断 dispatcher** | ghq 配下の任意 repo (`<host>/<owner>/<repo>`) | Claude / Codex |
| `claude agents` | **repo 内 dispatcher + bg viewer** | 開いたディレクトリ + その直接の子 (`@<repo>` で兄弟解決) | Claude のみ |

使い分けの目安:

- **ghq 配下の任意 repo に新規セッションを開きたい** → `tmux-sidebar new`
- **Codex CLI でセッションを開きたい** → `tmux-sidebar new`
- **既に開いている repo の中で並列タスクを並べたい** → `claude agents`
- **ターミナルから離れても走り続けるバックグラウンドセッションを使いたい** → `claude --bg` / `claude agents`

衝突しないこと:

- 状態ファイルは別パス。tmux-sidebar が `/tmp/agent-pane-state/pane_N`、Agent View が `~/.claude/jobs/<id>/state.json` と `~/.claude/daemon/roster.json` を使う。
- worktree も別パス。tmux-sidebar dispatch は `<main>@<branch-dirname>` (兄弟ディレクトリ)、Agent View の auto-worktree は `.claude/worktrees/<auto>` (project-local)。
- 同じ Claude プロセスが両方の view に同時に出ることはない。tmux pane 内で起動した `claude` は sidebar のみ、`claude --bg` で起動したものは Agent View のみ。

Agent View はリサーチプレビュー段階のため、コード変更を伴う統合 (auto-worktree の場所共有、`jobs/state.json` 直読、`claude --bg` への移行など) は GA を待って再評価する。経過観察の判断材料と再評価トリガは [`docs/history.md`](history.md) を参照。
