...
- [x] `internal/picker` パッケージを新設、bubbletea で repo→mode→config の wizard を実装
- [x] subcommand `tmux-sidebar new --context=<file>` を追加
- [x] `--context` の JSON フォーマット定義（既存 sessions / pinned / sidebar session id）

### 起動経路

- [x] sidebar pane で `N` 押下 → context を temp file へ書き出し → `tmux display-popup -E -w 80 -h 24 'tmux-sidebar new --context=...'`
- [x] popup 終了後、pane mode が `loadData()` を発火、新 session にカーソル移動

### Step 1: repo 選択

- [x] `internal/repo` に ghq 配下 repo 列挙を実装（`ghq list` 呼び出し）
- [x] fuzzy filter（subsequence match の独自実装。外部依存を増やさない）
- [x] context の既存 sessions と突き合わせ、open 中の repo を dim 表示
- [x] `Enter` で:
  - 未 open repo → mode 選択へ進む
  - open 中 repo → 既存 session へ switch して終了

### Step 2: prompt 入力（dispatch_launcher.fish と同 UX）

- [x] `internal/dispatch` パッケージを Go で実装（dispatch.sh の launch を移植）
- [x] `tmux-sidebar dispatch <repo> [prompt] [flags]` subcommand を追加（dispatch.sh と CLI 互換、構造化出力も同形式）
- [x] picker mode を 2-step (repo+launcher → prompt) に再編。mode 選択 step は廃止
- [x] Step 1 / Step 2 両方で Tab により claude ↔ codex toggle
- [x] Step 2 ヘッダーは `tab: モード切替 enter: 実行 :<branch> で既存 remote branch を checkout` + `claude / codex  <repo>` + 区切り線
- [x] branch 自動生成（先頭行 → `feat/<slug>`）と `:<branch>` プレフィックスでの checkout-mode を入力欄下にプレビュー
- 採用しなかった:
  - orchestrate mode（dotfiles 側でも実用に至っておらず YAGNI）
  - claude / codex 素起動 mode（picker は常に dispatch 経由。素起動は CLI `tmux-sidebar dispatch --no-prompt` か tmux native の `prefix+c`）

### 実行

- [x] tmw / agent 起動コマンドを `os/exec` で実行（tmw が repo 引数を取らない fish function なので、tmux new-session + send-keys で直接構築）
- [x] 失敗時は picker 内 error line に stderr を表示、`Esc` で popup 終了
- [x] 成功時は popup を閉じて pane mode に戻る

### 既存 popup tmw / dispatch_launcher との並存

- [x] dotfiles 側の popup tmw キーバインド（`prefix+s`）はそのまま fallback として残す
- [x] dotfiles 側の `prefix+S` (dispatch_launcher.fish) もそのまま残す。将来 dotfiles 側で `dispatch.sh` を `tmux-sidebar dispatch` の thin wrapper に書き換えれば engine が一本化される
- [x] README / setup.md にて「sidebar の `N` が primary、popup tmw / dispatch_launcher は fallback」と明記

---

## Phase 5: doctor 更新と documentation

- [x] `tmux-sidebar doctor` に tmux 3.2 以上の確認（popup 必須）を追加
- [x] README.md を Phase 進捗に応じて更新（features / keyboard shortcuts / subcommands）
- [x] docs/setup.md に Popup picker (`N`) の前提を追記（§10）
