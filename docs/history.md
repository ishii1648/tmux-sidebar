...
## running 経過時間の起点 (`pane_N_started`) リセット契機をターン境界に変更（2026-05-31）

### 背景

`internal/hook` は当初、`running` への遷移時に「直前の status が running でなければ `pane_N_started` を現在時刻に書き直す」設計だった（PR #57 / `issues/closed/0013` の Codex 対応もこの保持条件のまま）。

しかし Claude の推奨 hook 構成は `PreToolUse → running` / `PostToolUse → idle` / `Stop → idle` であり、複数 tool を使う 1 ターンの間に `running → idle → running → idle …` を繰り返す。2 個目以降の `running` は直前が `idle` と判定されるため、ユーザから見て連続作業中の 1 ターンでも tool ごとに `pane_N_started` がリセットされ、running バッジの経過時間が `0s` に戻っていた（`issues/closed/0015`）。

### 設計前提の反転

`pane_N_started` の意味を「直近の running 連続区間の起点」から「**1 ターン (running episode) の起点**」へ変更した。

- `hook running`: 既存の `pane_N_started` があれば prevStatus に関わらず保持し、無いときだけ作成する（保持条件から prevStatus 判定を撤廃）
- リセット契機を **ターン終了 (Stop)** に移し、`hook` サブコマンドに `--reset` フラグを追加。Stop hook を `tmux-sidebar hook idle --reset`（Codex は `--kind codex --reset`）に変更して、ターン終了時だけ `pane_N_started` を削除する

Stop の idle と PostToolUse の idle は同じ `idle` status を書くため、書き込み時点では区別できない（次に来る event でしか判別できない）。よって「ターン終了」という lifecycle 情報を `--reset` フラグとして hook 呼び出し側に明示させる方式を採った。doctor は旧 `Stop: hook idle` を検出し、同一 (status, kind) の旧コマンドを `--reset` 付きに置換する（重複させない）。

### 採用しなかった代替

- **Claude でも `PostToolUse → idle` をやめる（Codex と同じ方針）**: tool 間の idle を書かなければ running が連続し、prevStatus 判定のままでもリセットは起きない。しかし tool 間の「考え中」も running 表示になり、idle バッジが一切出なくなる。tool 間に idle を出す現挙動は維持したかったため却下した。
- **session_id / UserPromptSubmit でターン境界を取る**: session_id はセッション内で安定しターン境界にならない。UserPromptSubmit でリセットする案は、doctor が deprecated 扱いしている UserPromptSubmit hook を再導入することになり整合しないため見送った。

### 影響

- running 経過時間はターンの最初の running から Stop までを通して計測される（tool 間の idle blip では中断されない）。permission / ask 待ちを跨いでも保持される。
- 既存ユーザは `doctor --yes` で Stop hook が `--reset` 付きに upgrade される。upgrade 前のバイナリ + `--reset` なし設定では従来どおり（tool ごとにリセット）動作するため、後方互換性は壊れない。

## Claude の `PostToolUse` を `idle` から `running` に変更（2026-05-31）

### 背景

直前の節（`pane_N_started` のリセット契機変更, `issues/closed/0015`）では、経過時間のリセットだけを直し、`PostToolUse → idle` は「tool 間に idle バッジを出す現挙動を維持したい」という前提で温存した（同節の「採用しなかった代替」で `PostToolUse → idle` 廃止案を却下している）。

しかし review-loop 後のユーザ指摘で、この前提自体に欠陥があることが判明した。`PostToolUse → idle` を残すと、ユーザから見て連続作業中の 1 ターンでも tool 完了のたびにバッジが `running → idle → running` と flicker する。経過時間は保持されても status バッジは idle に戻る。さらに `docs/spec.md` の destructive 操作（`d`/`D`）の confirm 強度判定が tool 間の idle 瞬間に「running 中なのに単純確認」になり、spec の意図に反する。

### 設計前提の反転

「tool 間に idle を出したい」という前提を捨て、Claude の hook を「**ターン中 = running、ターン終了 (Stop) = idle**」に統一した。具体的には `PostToolUse` の書き込みを `idle` から `running` に変更した（`PreToolUse` と同じ）。

- tool 間: `PreToolUse(running)` → tool → `PostToolUse(running)` → 生成 → 次 `PreToolUse(running)`。ずっと running で flicker しない
- permission: Claude の permission は `Notification`（`permission_prompt`）が `💬` を書く（`issues/0011`）。`PostToolUse → idle` は tool 完了時にこの `💬` をクリアするアンカーも兼ねていた。`PostToolUse → running` でも同様にクリアされ、running に戻る
- idle は `Stop(idle --reset)` のみ。0015 の started 保持 / `--reset` とそのまま両立する

### 採用しなかった代替

- **`PostToolUse` を削除する（Codex と同型）**: flicker は解消するが、`PostToolUse → idle/running` のアンカーが消えるため、`Notification` で書いた permission(`💬`) を tool 完了時にクリアできず、次の `PreToolUse(running)` か `Stop` まで張り付く（承認 tool がターン最後だと生成中ずっと `💬`）。permission の正確さを優先して却下。`PostToolUse → running` なら flicker 解消と permission クリアを両立できる。
- Codex への適用は見送り。Codex は `PermissionRequest` で permission を出せており、`PostToolUse` を持たない issue 0013 の方針を今回は尊重した（Codex 側の permission 張り付きは別途検討）。

### 影響

- スコープは Claude のみ。doctor は Claude の `PostToolUse` 必須コマンドを `tmux-sidebar hook running` に変更。`upsertHookGroup` の置換ロジックを「canonical 以外の `tmux-sidebar hook` コマンドを全除去」に強化し、`idle → running` の status 跨ぎ移行でも pane_N に 2 つの writer が残らないようにした。
- 既存ユーザは `doctor --yes` で `PostToolUse` が `idle`（または旧 `idle --reset`）から `running` に置換される。

## running 経過時間の起点リセットに `UserPromptSubmit`（ターン開始）を追加（2026-06-02）

### 背景

`pane_N_started`（running 経過時間の起点）のリセット契機を「ターン終了 (Stop) の `idle --reset` だけ」に置く設計（2026-05-31「running 経過時間の起点リセット契機をターン境界に変更」）は、Stop hook が**必ず発火する**ことを暗黙の前提にしていた。同節の「採用しなかった代替」で `UserPromptSubmit` でのリセット案を「doctor が deprecated 扱いしている UserPromptSubmit hook を再導入することになり整合しない」として却下している。

しかし実運用で、この前提が崩れるケースが顕在化した（`issues/0018`）。**Esc / Ctrl-C による中断や Claude のクラッシュでは Stop hook が発火しない**ため、`pane_N_started` が削除されないまま残る。次のターンの最初の `running` は既存の起点を保持するので、`now − 前ターンの起点` がそのまま経過時間になり、間の離席時間ごと累積して巨大な値（実機で 256 分=4h16m を観測）を表示していた。Stop 設定自体は `idle --reset` で正しく入っており、設定ミスではなく「Stop が飛ばないターン終了」が原因。

### 設計前提の反転

「起点を区切るのは Stop だけ」という前提を捨て、**ターン開始 (`UserPromptSubmit`)** にも起点リセットを置いた。新しいユーザ入力は確実なターン境界なので、前ターンがどう終わっても次ターン頭で起点が張り直り、中断・クラッシュに構造的に強くなる。

- `hook` の `--reset`（`Options.ResetElapsed`）のセマンティクスを status 依存に拡張:
  - 非 running（`idle --reset`、Stop）: `pane_N_started` を削除（ターン終了、従来どおり）
  - running（`running --reset`、UserPromptSubmit）: `pane_N_started` を「今」に上書き（ターン開始の再アンカー、新規）
- doctor の `requiredClaudeHooks` に `UserPromptSubmit = tmux-sidebar hook running --reset` を追加。2026-05-03「agent-pane-state を hook サブコマンドに集約」で導入し、その後 PreToolUse 正準化で legacy 扱いにした `UserPromptSubmit` の deprecation（`checkLegacyClaudeHooks`「superseded by PreToolUse — safe to remove」）を撤回し、関数ごと削除した。
- Stop の `idle --reset` は二重防御として残す（A 採用後はターン頭で必ず張り直るため correctness は Stop に依存しなくなるが、ターン間に起点を残さない衛生のため保持）。

### 採用しなかった代替

- **reader 側 stale ガード（`internal/state` で経過時間に上限・妥当性チェック）**: hook を増やさず即効で巨大値の露出を抑えられるが、単一スナップショットでは「4 時間連続の正規ターン」と「4 時間前に中断して再開した leak」が同一シグネチャ（`started` が古く `pane_N` mtime が新しい）になり区別できない。閾値ベースのヒューリスティックにしかならず本質的な解にならないため、本筋には採らなかった（安全網としては併用可だが今回は未実装）。
- **`SessionStart` でリセット**: セッション単位で per-turn ではない。1 セッション内で中断ターンを繰り返すと leak が残るため単独では不十分。
- **`running` 書き込み時に「前回が終端 idle なら張り直す」**: Stop が発火しない中断ケースには終端 idle マーカーが残らず、`PostToolUse → running`（2026-05-31）で mid-turn idle も消えているため、状態遷移から境界を復元できない。
- **Stop 依存のまま現状維持 + ドキュメント注意書き**: 中断は日常操作で運用回避できないため、バグとして残す選択肢にならない。

### 影響

- スコープは Claude のみ。Codex は `UserPromptSubmit` を required に加えていない（issue 0013 / 2026-05-31 の Codex 据え置き方針を継続）。`hook` 本体は kind 非依存なので、Codex ユーザが手動で `running --reset --kind codex` を `UserPromptSubmit` に設定すれば同じ恩恵を受けられる。
- `running --reset` が再アンカーするようになったため、`--reset` が誤って `PreToolUse` / `PostToolUse` に紛れ込むと tool ごとに起点が 0 に戻る（0015 の再来）。doctor の正規化 exact-compare による mismatch 検出がこれを WARN + auto-fix で弾く。
- 既存ユーザは `doctor --yes` で `UserPromptSubmit` に `running --reset` が追加され、`running`（`--reset` なし）が設定済みなら `running --reset` に upgrade される。
