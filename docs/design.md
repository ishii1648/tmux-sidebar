...
- `pane_N_started`: running 開始 epoch 秒。1 ターンの running episode を通して保持する。`hook running` は既存値があれば保持し、無いときだけ書く。ターン終了の Stop hook（`hook idle --reset`）だけがこれを削除し、次の running が新しい起点を作る

Claude の hook → status 対応は「ターン中 = running、ターン終了 = idle」に揃える。`PreToolUse` / `PostToolUse` の両方が `running` を書き（tool 実行中も生成中も running 維持）、`Notification`（`permission_prompt`）が `permission` を、`Stop` が `idle --reset` を書く。`PostToolUse` を `idle` にすると tool 間でバッジが flicker し、permission(`💬`) を tool 完了後にクリアできず張り付くため `running` を採用している（経緯は history 参照）。Codex は `PreToolUse=running` / `PermissionRequest=permission` / `Stop=idle --reset` で、`PostToolUse` は設定しない（issue 0013）。
...
| `hook <status> [--kind ...] [--reset]` | agent (Claude Code / Codex) hook から呼ばれて状態ファイルを書き出す。`internal/hook` への薄い wrapper。`--reset` は `pane_N_started` を削除し running 経過時間をターン境界で区切る（Stop hook 用） |
