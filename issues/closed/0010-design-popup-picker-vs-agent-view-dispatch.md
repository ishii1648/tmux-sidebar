...
- [x] `docs/spec.md` に「Agent View との分担」節を追加 (使い分け早見表、住み分け宣言)
- [x] `docs/history.md` に経過観察方針を append (#0007〜#0009 の pending 化と並べて、Agent View 全体への姿勢として記録)
- [ ] `README.md` の Features 説明を必要に応じて補強 — spec への導線は Documentation 節で十分カバー、Features 追記は見送り
- [x] 再評価トリガを spec / history に明記 (Agent View GA、`@<host>/<owner>/<repo>` の階層解決対応、`disableAgentView` の市場浸透度)

## 解決方法

`docs/spec.md` の「Coexistence with tmux native bindings」直後に **「Agent View (`claude agents`) との分担」** 節を新設した。`tmux-sidebar new` = repo 横断 dispatcher、`claude agents` = repo 内 dispatcher の住み分け、使い分け早見表、衝突しないリソース (状態ファイル / worktree / settings.json hook) を明文化し、リサーチプレビュー期間中の経過観察方針へのリンクを `docs/history.md` に張った。

`docs/history.md` には「Claude Code Agent View 登場時の方針判断（2026-05-13）」を append し、4 つの検討候補 (#0007〜#0010) の判定、採用しなかった代替 (Agent View 一本化 / `claude --bg` wrapper / ghq 全廃) とその却下理由、衝突しないことを確認した点、再評価トリガを記録した。

`README.md` の Features 補強は任意項目だったが、README の Documentation 節に既に `docs/spec.md` への導線があり、Agent View との関係は spec を読めば分かる構造なので追記は見送り。

`#0007` / `#0008` / `#0009` は同じ 2026-05-13 の判断で `issues/pending/` に格納済み。
