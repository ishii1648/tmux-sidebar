...
- `pane_N_started`: running 開始 epoch 秒。1 ターンの running episode を通して保持し、tool ごとの idle blip を跨いでも維持する。`hook running` は既存値があれば保持し、無いときだけ書く。ターン終了の Stop hook（`hook idle --reset`）だけがこれを削除し、次の running が新しい起点を作る
