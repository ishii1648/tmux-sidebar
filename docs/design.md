...
- `pane_N_started`: running 開始 epoch 秒。1 ターンの running episode を通して保持する。`hook running`（`--reset` なし）は既存値があれば保持し、無いときだけ書く。起点を区切る境界は 2 つ: **ターン開始**の `UserPromptSubmit`（`hook running --reset`）が起点を「今」に張り直し、**ターン終了**の `Stop`（`hook idle --reset`）が削除する。`UserPromptSubmit` での再アンカーにより、前ターンが Esc 中断・クラッシュで Stop を発火せず終わっても、次ターンが古い起点を引き継いで経過時間が累積することを防ぐ（issues/0018）

Claude の hook → status 対応は「ターン開始/ターン中 = running、ターン終了 = idle」に揃える。`UserPromptSubmit` が `running --reset`（ターン起点の再アンカー）、`PreToolUse` / `PostToolUse` の両方が `running` を書き（tool 実行中も生成中も running 維持）、`Notification`（`permission_prompt`）が `permission` を、`Stop` が `idle --reset` を書く。`PostToolUse` を `idle` にすると tool 間でバッジが flicker し、permission(`💬`) を tool 完了後にクリアできず張り付くため `running` を採用している（経緯は history 参照）。Codex は `PreToolUse=running` / `PermissionRequest=permission` / `Stop=idle --reset` で、`PostToolUse` は設定しない（issue 0013）。
...
| `hook <status> [--kind ...] [--reset]` | agent (Claude Code / Codex) hook から呼ばれて状態ファイルを書き出す。`internal/hook` への薄い wrapper。`--reset` は running 経過時間をターン境界で区切る: `idle --reset` は `pane_N_started` を削除（Stop hook 用）、`running --reset` は「今」に張り直す（UserPromptSubmit hook 用） |
